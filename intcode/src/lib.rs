...
pub type Value = i128;
pub type RawProgram = HashMap<usize, Value>;
...
impl From<Value> for SingletonValue {
  fn from(value: Value) -> Self {
    Self { inner: value }
  }
}

impl std::ops::Deref for SingletonValue {
  type Target = Value;
...
impl std::ops::DerefMut for SingletonValue {
  fn deref_mut(&mut self) -> &mut Self::Target {
    &mut self.inner
  }
}

impl Output for SingletonValue {
  fn write(&mut self, output: Value) {
    **self = output;
  }
}
...
pub fn file_to_values(file: File) -> Vec<Value> {
